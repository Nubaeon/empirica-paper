...
\begin{tikzpicture}[
    node distance=1.5cm,
    phase/.style={rectangle, draw, rounded corners, minimum width=2.4cm, minimum height=0.9cm, fill=blue!10, font=\small\bfseries},
    decision/.style={diamond, draw, aspect=2.5, fill=yellow!20, font=\small},
    action/.style={rectangle, draw, minimum width=2cm, minimum height=0.8cm, fill=green!10, font=\small},
    update/.style={rectangle, draw, rounded corners, minimum width=2.4cm, minimum height=0.9cm, fill=orange!15, font=\small\bfseries},
    label/.style={font=\scriptsize, text=gray},
    arrow/.style={-{Stealth[length=2.5mm]}, thick}
]
    % Main phases - top row
    \node[phase] (preflight) {PREFLIGHT};
    \node[phase, right=2.5cm of preflight] (check) {CHECK};
    \node[phase, right=2.5cm of check] (postflight) {POSTFLIGHT};

    % Labels under phases
    \node[label, below=0.15cm of preflight] {Baseline};
    \node[label, below=0.15cm of check] {Sentinel Gate};
    \node[label, below=0.15cm of postflight] {Learning Delta};

    % Decision diamond - below CHECK
    \node[decision, below=2cm of check] (proceed) {PROCEED?};

    % Action boxes - side by side below decision
    \node[action, below left=1.2cm and 1.5cm of proceed] (praxic) {PRAXIC};
    \node[action, below right=1.2cm and 1.5cm of proceed] (noetic) {NOETIC};
    \node[label, below=0.15cm of praxic] {(execute)};
    \node[label, below=0.15cm of noetic] {(investigate)};

    % Bayesian update - bottom center, below the action boxes
    \node[update, below=4.5cm of check] (bayesian) {Bayesian Update};

    % Arrows - main flow
    \draw[arrow] (preflight) -- (check);
    \draw[arrow] (check) -- (postflight);

    % Arrow from CHECK down to decision
    ...
    % Praxic goes to postflight (curved path going right and up)
    \draw[arrow] (praxic.east) -- ++(0.5,0) |- ([yshift=-0.3cm]postflight.south) -- (postflight.south);

    % Noetic loops back to check (curved path going left and up)
    \draw[arrow] (noetic.west) -- ++(-0.5,0) |- ([yshift=0.5cm]proceed.north) -- (proceed.north);

    % Postflight to Bayesian
    \draw[arrow] (postflight.south) -- ++(0,-1) -| (bayesian.north);
