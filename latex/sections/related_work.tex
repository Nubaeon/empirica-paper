...
This is not a criticism of RLHF---reducing harmful overconfidence is valuable. Rather, it suggests that alignment training produces systematic biases that can be measured and corrected. The learning delta we observe (+0.162 mean capability growth during tasks, with 83.3\% of learning pairs showing improvement) may partially reflect AI systems overcoming training-induced conservatism through task-specific evidence.
...
\subsection{Epistemic Measurement in AI}

A question for any AI self-assessment framework is whether the measurements track something real. Recent work on emergent introspective awareness \citep{bricken2025introspection} finds that models possess ``a limited, functional form of introspective awareness'' but that ``abilities observed are highly unreliable, and failures of introspection remain the norm.'' \citet{steyvers2025metacognition} note that LLM metacognitive abilities are ``limited in resolution, emerge in context-dependent manners, and seem qualitatively different from humans.''

Our framework is compatible with these findings because it does not require reliable introspection. We measure information state: what the system knows relative to a task, what it can do, how complete its context is. These are structured prompts that produce numeric outputs---the same mechanism as when an AI estimates confidence, verifies facts, or scores relevance. The outputs are useful when they correlate with task-relevant states and calibrate over time.

The empirical evidence supports this: systematic calibration patterns across 1,190,265 observations (\Cref{sec:results}), 107$\times$ variance reduction with evidence accumulation, and grounded verification showing that self-assessed vectors track objective outcomes for capability dimensions. We don't assume perfect measurement---we measure systematic biases and correct for them through dual-track calibration. What these measurements reveal about the system's internal processes is a separate question this paper does not address.
