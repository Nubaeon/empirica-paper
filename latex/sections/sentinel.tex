...
uncertainty & $-$0.106 \\
completion & +0.601 \\
engagement & +0.010 (near-calibrated) \\
\bottomrule
\end{tabular}
...
\subsubsection{Domain-Specific Configuration}

The thresholds reported in this paper (KNOW $\geq$ 0.70, UNCERTAINTY $\leq$ 0.35) are our production defaults for software engineering tasks. These are not universal constants---they are domain-dependent parameters that must be tuned to the risk profile and feedback characteristics of each application domain:
...
\bottomrule
\end{tabular}
\caption{Domain-specific threshold configurations. Production defaults (KNOW $\geq$ 0.70, UNCERTAINTY $\leq$ 0.35) are for software engineering; other domains require calibration to local risk profiles.}
\label{tab:domain-thresholds}
\end{table}

\paragraph{Extensible Vector Space}

The 13 epistemic vectors described in this paper constitute a base set validated for software engineering. Different domains may require \emph{extended vectors} that capture domain-specific epistemic dimensions. For example, legal domains might add PRECEDENT\_COVERAGE and JURISDICTIONAL\_CONFIDENCE; medical domains might add CONTRAINDICATION\_AWARENESS. The framework's architecture supports additional vectors---the Sentinel evaluates whatever vectors are present in the submitted assessment, and the Bayesian calibration system tracks each vector independently. Domain-specific vectors would accumulate their own calibration history and correction terms.
...
From production deployment (792 sessions):
...
\midrule
Total CHECK submissions & 683 \\
Total cascades & 616 \\
Clean learning pairs & 456 \\
Knowledge improvement rate & 83.3\% \\
Uncertainty reduction rate & 83.3\% \\
\bottomrule
\end{tabular}
...
The Sentinel gate does more than return a binary proceed/investigate decision---it functions as a \textbf{capability-based access control layer} that determines which tools and actions are available in each epistemic phase.

\subsubsection{Phase-Dependent Capabilities}

Via hooks that intercept tool invocations before execution, the framework enforces different capability sets for noetic and praxic phases:

\begin{table}[h]
\centering
\begin{tabular}{lll}
\toprule
\textbf{Phase} & \textbf{Allowed Actions} & \textbf{Blocked Actions} \\
\midrule
Noetic (investigating) & Read, search, explore, ask & Write, edit, commit, deploy \\
Praxic (executing) & All actions (full capability set) & --- \\
No session (ungated) & Read-only & All mutations \\
\bottomrule
\end{tabular}
\caption{Capability sets by epistemic phase.}
\label{tab:capability-acl}
\end{table}

This is analogous to a Linux Security Module (LSM): the Sentinel gate evaluates the epistemic state at each tool invocation and grants or denies capabilities accordingly. The AI cannot write code during the noetic phase---not because a filter catches bad output, but because the capability to write is not available until the gate confirms sufficient knowledge.

\subsubsection{Hook-Based Enforcement}

The access control is enforced through pre-tool-use hooks that execute before each action:

\begin{enumerate}
    \item The AI invokes a tool (e.g., file edit, bash command)
    \item The hook checks the current epistemic phase and session state
    \item If the action exceeds the phase's capability set, it is blocked with an explanation
    \item The AI must either investigate further (noetic) or submit to CHECK (gate transition)
\end{enumerate}

This creates a security boundary between knowing and doing. The AI's epistemic state determines its permissions, and permissions determine what outcomes are possible. Miscalibrated self-assessment can still occur, but it cannot bypass the capability gate---the AI must convince the Sentinel, not just itself.

The capability gate operates within a larger epistemic ecosystem: at PREFLIGHT, semantic retrieval surfaces relevant lessons and prior findings; during the noetic phase, the cognitive immune system challenges stale knowledge; intent edges track which investigation produced which action. The Sentinel is the enforcement mechanism, but the knowledge infrastructure surrounding it---the learning pipeline described in \Cref{sec:learning-pipeline}---is what makes the gate decisions meaningful.
...
\subsubsection{Adaptive Autonomy Nudges}

An earlier version of the framework experimented with automatic POSTFLIGHT triggers (auto-POSTFLIGHT) and automatic CHECK submissions (auto-CHECK). Both were removed because they create perverse incentives: if the AI knows that high COMPLETION triggers automatic measurement, it can game the vector to control when measurement occurs. More fundamentally, automatic PREFLIGHT and POSTFLIGHT would break the calibration system---these are measurement boundaries that must reflect genuine self-assessment, not automated triggers.

Instead, the Sentinel implements \textbf{adaptive nudges}: informational signals that suggest POSTFLIGHT based on transaction length relative to the AI's historical patterns. The system tracks the AI's average tool-call count across its last 20 POSTFLIGHT records and computes nudge thresholds:

\begin{table}[h]
\centering
\begin{tabular}{lll}
\toprule
\textbf{Ratio to Average} & \textbf{Nudge Level} & \textbf{Message} \\
\midrule
$\geq 1.0\times$ & Info & ``Past average. Natural POSTFLIGHT point.'' \\
$\geq 1.5\times$ & Warning & ``Consider POSTFLIGHT soon.'' \\
$\geq 2.0\times$ & Strong & ``POSTFLIGHT strongly recommended.'' \\
\bottomrule
\end{tabular}
\caption{Adaptive POSTFLIGHT nudge thresholds. Nudges are informational---the AI decides when to close the transaction based on work coherence, not the nudge level.}
\label{tab:nudge-thresholds}
\end{table}

These nudges are advisory, not enforced. The AI retains agency over when to close a transaction, preserving the integrity of the measurement boundary. Delegated subagent tool calls are counted toward the parent's total, ensuring that heavy delegation also triggers appropriate nudges. This approach balances autonomy with measurement discipline: transactions that run too long lose measurement fidelity, but the decision to close remains with the AI.
...
The empirical result: calibrated gating reduces error rates by more than 50\% compared to raw self-assessment. The learning delta (\Cref{sec:results}) shows that investigation produces genuine growth in effective capability---the externalized scaffolding captures and compounds what the AI discovers. The gate doesn't just prevent bad outputs; it enables good ones.
