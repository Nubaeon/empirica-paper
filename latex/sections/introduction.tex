...
\begin{enumerate}
    ...
    \item \textbf{The CASCADE Workflow}: A 4-phase loop---PREFLIGHT $\rightarrow$ CHECK $\rightarrow$ POSTFLIGHT $\rightarrow$ POST-TEST---that captures epistemic state before and after task execution, measures learning, and calibrates future assessments through dual-track Bayesian updates.

    \item \textbf{Grounded Verification}: Post-test evidence collection that compares self-assessed epistemic state against objective outcomes (test results, git metrics, goal completion), anchoring calibration in what actually happened rather than further self-report.
\end{enumerate}
